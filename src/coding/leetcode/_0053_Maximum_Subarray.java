...
public class _0053_Maximum_Subarray {
    ...
    public int maxSubArray_DP(int[] nums) {
        ...
        int maxCurr = nums[0]; // Store the current max sum of subarray
        int maxSum = nums[0];
        for (int i = 1; i < nums.length; ++i) {
            maxCurr = Math.max(nums[i], maxCurr + nums[i]); // Whether the current number should start a new subarray
            maxSum = Math.max(maxSum, maxCurr); // Mark down current max sum of subarray
        }
        return maxSum;
    }
